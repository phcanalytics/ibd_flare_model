CREATE VOLATILE TABLE ibd_enc_count AS
	...
	SELECT ptid, encid, SUM(uc_flag) AS uc_count, SUM(cd_flag) AS cd_count
		...
		SELECT ptid, encid, diag_date, diagnosis_cd_type, 
			...
			CASE WHEN (diagnosis_cd_type = 'ICD9' AND diagnosis_cd LIKE '556%') OR
				(diagnosis_cd_type = 'ICD10' AND diagnosis_cd LIKE 'K51%')
				THEN 1 ELSE 0 END uc_flag,
			-- Make new flag variable for cd
			CASE WHEN (diagnosis_cd_type = 'ICD9' AND diagnosis_cd LIKE '555%') OR
				(diagnosis_cd_type = 'ICD10' AND diagnosis_cd LIKE 'K50%')
