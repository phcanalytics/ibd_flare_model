"""
Title: Sensivitiy Random Forest Model with MICE imputation to Predict Flare
Author: Ryan Gan
Date Created: 2019-10-14

Running model with longitudinal lab summaries and previous flare included. 

Key difference is MICE imputation used instead of simple median imputation.
...
# using ridge in iterative imputer instead of extra trees
# as it is faster with similar performance
...
train = pd.read_csv(data_path, dtype = {'birth_yr': str, 'date_of_death': str})
...
Preprocessing Steps:
1. Define numeric pipeline which imputes missing labs with iterative chain imputation
using bayesian ridge. Note this is MICE imputation.
...
num_pipe = Pipeline(
    [('num_selector', FeatureSelector(feature_names = num_cols)),
     # mice itterative imputation with bayesian ridge
     ('iterative_imputer', IterativeImputer(random_state=0,
                                            estimator=BayesianRidge()))#,
     #('std_scaler', StandardScaler()) # take out standardizer for interpretability
    ])
...
bayes_prob = [bayes(obs_pred=x, pop_prop=0.115) for x in y_pred[:,1]]
...
results_folder = root_dir + '/results/rf_mice/'
...
# save roc_df
roc_df.to_csv(results_folder + "rf_mice_roc.csv")
# save auc
print(roc_auc, 
...
plt.savefig(results_folder + "rf_mice_roc.pdf", facecolor = 'white')
...
vif_df.to_csv(results_folder + 'rf_mice_vif.csv')
...
plt.savefig(results_folder + "rf_mice_vif_plot.pdf", 
            facecolor = 'white')
...
print(class_report,
    file=open(results_folder + 
              "rf_mice_classification_report.txt",
              "w"))
...
# run accuracy summary on confuxion matrix
rf_mice_dx_summary = dx_accuracy(cm)

print(rf_mice_dx_summary)
# save summary metrics
rf_mice_dx_summary.to_csv(results_folder + "rf_mice_dx_summary.csv")
...
# save auc
print(roc_auc, 
      file=open(results_folder + "rf_mice_roc_auc.txt", "w"))
...
# save roc plot; pass white facecolor to save
plt.savefig(results_folder + "rf_mice_roc_plot.pdf", 
            facecolor = 'white')
...
if run_boot==True:
    ...
    # save dx_intervals
    dx_intervals.to_csv(results_folder + "rf_mice_dx_intervals.csv")
